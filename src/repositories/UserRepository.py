...
from ..tables import User, TypeUser, FaceUser
...
class UserRepository:
    ...
    async def add_face_user(self, face_user: FaceUser):
        try:
            self.__session.add(face_user)
            await self.__session.commit()
            return face_user
        except:
            await self.__session.rollback()
    ...
    async def get_face_user_by_embedding(self, embedding) -> int:
        threshold = 10
        query = text(f""" 
        select id, distance
        from (
        select id, sqrt(sum(distance)) as distance
        from (
            select id, pow(unnest(embedding) - unnest(ARRAY{embedding}), 2) as distance
            from face_user
        ) sq
        group by id
    ) sq2
    ...
    async def get_user_by_face_id(self, face_id: int) -> User:
        query = select(User).where(User.id_face_user == face_id)
        response = await self.__session.execute(query)
        return response.scalars().first()